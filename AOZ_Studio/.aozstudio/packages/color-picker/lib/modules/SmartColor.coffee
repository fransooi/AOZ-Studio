...
# ----------------------------------------------------------------------------
    ...
    # -------------------------------------
        COLOR_REGEXES =
            ...
            # Matches AOZ:
            # eg. #000 and #ffffff
            AOZ: /(\$[a-f0-9]{6}|\$[a-f0-9]{3})/i
			
        MATCH_ORDER = ['AOZ', 'HSL', 'HSLA', 'HSV', 'HSVA', 'VEC', 'VECA', 'RGB', 'RGBA', 'HEXA', 'HEX']
    ...
    # -------------------------------------
        ...
        # -------------------------------------
            ...
            # ---------------------------
                toAOZ: ->
                    _aoz = Convert.rgbToAoz @RGBAArray

                    # Abbreviate if `abbreviateValues` option is true
                    if atom.config.get 'color-picker.abbreviateValues'
                        if _aoz[0] is _aoz[1] and _aoz[2] is _aoz[3] and _aoz[4] is _aoz[5]
                            _aoz = "#{ _aoz[0] }#{ _aoz[2] }#{ _aoz[4] }"

                    # Uppercase color values if `uppercaseColorValues` option is true
                    if atom.config.get 'color-picker.uppercaseColorValues'
                        _aoz = _aoz.toUpperCase()
        ...
        # -------------------------------------
			...
			# AOZ
            AOZ: (value) -> @color 'AOZ', value, do ->
                return (Convert.aozToRgb value).concat [1]
