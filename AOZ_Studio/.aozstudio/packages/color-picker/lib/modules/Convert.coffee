...
# ----------------------------------------------------------------------------
    ...
    # -------------------------------------
        aozToRgb: (aoz) ->
            aoz = aoz.replace '$', ''
            aoz = aoz.replace /(.)(.)(.)/, "$1$1$2$2$3$3" if aoz.length is 3

            return [
                parseInt (aoz.substr 0, 2), 16
                parseInt (aoz.substr 2, 2), 16
                parseInt (aoz.substr 4, 2), 16]
    ...
    # -------------------------------------
        aozToHsl: (aoz) ->
            return @rgbToHsl @aozToRgb aoz.replace '$', ''
    ...
    # -------------------------------------
        rgbToAoz: (rgb) ->
            _componentToHex = (component) ->
                _aoz = component.toString 16
                return if _aoz.length is 1 then "0#{ _aoz }" else _aoz
