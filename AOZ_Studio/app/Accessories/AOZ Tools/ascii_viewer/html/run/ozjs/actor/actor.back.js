...
function copyActorImage( actor, imageName )
{
		...
		if ( imageCopy == undefined )
	{
		...
		//bob.hotSpot.x = actor.HotspotX;
		//bob.hotSpot.y = actor.HotspotY;
...
function set_actor( args )
{
	...
	if( args.behavior != undefined && args.behavior != '-255' )
	{
		...
		for( var b = 0; b < ba.length; b++ )
		{
			...
			if( parts )
			{
				...
				{
					application.aoz.runProcedure( behaviorObj.initProc, { BEHAVIOR$: parts[ 0 ], INDEX: !isNaN( bobParam.Id ) ? bobParam.Id : -1, INDEX$: '' + bobParam.Id }, bobParam );
				}
	...
	/**	
	copyActorImage( bobParam, args.Image );
	self.aoz.currentScreen.setModified();
	self.aoz.currentScreen.bobsToUpdate = true;
	bob.update( { force: true } );
	bob.toUpdateCollisions = true;
	...
	*/		
	...
	if( args.StartX != args.EndX || args.StartY != args.EndY || args.StartAlpha != args.EndAlpha || args.StartScaleX != args.EndScaleX || args.StartScaleY != args.EndScaleY || args.StartSkewX != args.EndSkewX || args.StartSkewY != args.EndSkewY || args.StartAngle != args.EndAngle )
	{
		...
		args.tween.addEventListener( "complete", function( event )
		{
			...
			if( args.OnChange && args.OnChange != "" )
			{
				self.aoz.runProcedure( args.OnChange, { EVENT$: "oncomplete", INDEX: !isNaN( args.Id ) ? args.Id : -1, INDEX$: '' + args.Id, X: bob.vars.x, Y: bob.vars.y, ALPHA: bob.vars.alpha, ANGLE: ( args.EndAngle ), SCALEX: bob.vars.scaleX, SCALEY: bob.vars.scaleY, HREV: bob.vars.hRev, VREV: bob.vars.vRev, SKEWX: bob.vars.skewX, SKEWY: bob.vars.skewY  }, args );
			}
		}, false );
	}
	...
	*/
	...
	if( bob2Param.behaviors && bob2Param.Enable )
	{
		for( var b in bob2Param.behaviors )
		{
			...
			if( behavior != undefined && behavior.state != undefined && behavior.state == 1 )
			{
				...
				{
					application.aoz.runProcedure( behavior.procedure, { EVENT$: 'onbehavior', CONTROL$: behavior.key, BEHAVIOR$: b,  STATE: behavior.state, INDEX: isNaN( bob2.Index ) ? -1 : bob2.Index, INDEX$: '' + bob2.Index }, bob2Param );
				}
			}
				...
				if( frame && Date.now() - bob2Param.animTempo >= frame.delay )
				{
					...
					if( bob2Param.animDirection == 0 )
					{
						...
						if( bob2Param.currentAnimFrame == bob2Param.currentAnimation.frames.length )
						{
							...
							{
								self.aoz.runProcedure( bob2Param.OnAnimChange, { EVENT$: 'onanimcomplete', INDEX: isNaN( index ) ? -1 : index, INDEX$: '' + index, ANIM$: bob2Param.Anim, FRAME: bob2Param.currentAnimFrame, TOTALFRAMES: bob2Param.currentAnimation.frames.length }, bob2Param );							
							}
					...
					if( bob2Param.OnAnimChange != undefined && bob2Param.OnAnimChange != '' )
					{
						self.aoz.runProcedure( bob2Param.OnAnimChange, { EVENT$: 'onanimchange', INDEX: isNaN( index ) ? -1 : index, INDEX$: '' + index, ANIM$: bob2Param.Anim, FRAME: bob2Param.currentAnimFrame, TOTALFRAMES: bob2Param.currentAnimation.frames.length }, bob2Param );							
					}
					...
					//copyActorImage( bob2Param, frame.index );
		...
		for( var c = 0; c < self.childrens.length; c++ )
		{
			...
			if( testBob && testParam && testParam.Id && bob2.index != testParam.Id && testParam.Collision && testParam.Visible && testParam.Enable && testBob.vars.alpha > 0 )
			{
				if( testBob )
				{
					...
					if ( coll )
					{
						self.aoz.runProcedure( bob2Param.OnCollision, { EVENT$: 'oncollision', INDEX1: !isNaN( bob2.Index ) ? bob2.Index : -1, INDEX1$: '' + bob2.Index, INDEX2: !isNaN( testBob.Index ) ? testBob.Index : -1, INDEX2$: '' + testBob.Index, IMAGE: !isNaN( testBob.vars.image ) ? testBob.vars.image : -1 , IMAGE$: '' + testBob.vars.image, GROUP1$: bob2Param.Group, GROUP2$: testParam.Group, ANIM1$: bob2Param.Anim, ANIM2$: testParam.Anim  }, bob2Param );
					}
				}
			}
		}
	...
	if( bob2Param.sprite && bob2Param.Enable && !self.paused && bob2Param.display )
	{
		...
		if( bob2Param.tmAnim != -1 && bob2Param.Anim != undefined  && bob2Param.display.spriteSheet && bob2Param.display.spriteSheet._data && bob2Param.display.spriteSheet._data[ bob2Param.Anim ] )
		{
			...
			if( bob2Param.currentFrame > ( bob2Param.display.spriteSheet._data[ bob2Param.Anim ].frames.length - 1 ) )
			{
				...
				{
					self.aoz.runProcedure( bob2Param.OnAnimChange, { EVENT$: 'onanimcomplete', INDEX: isNaN( bob2.Index ) ? -1 : bob2.Index, INDEX$: '' + bob2.Index, ANIM$: bob2Param.Anim, FRAME: bob2Param.currentFrame, TOTALFRAMES: bob2Param.display.spriteSheet._data[ bob2Param.Anim ].frames.length }, bob2Param );
				}
			...
			else
			{
				...
				if( bob2Param.OnAnimChange && bob2Param.OnAnimChange != "" )
				{
					self.aoz.runProcedure( bob2Param.OnAnimChange, { EVENT$: 'onanimchange', INDEX: isNaN( bob2.Index ) ? -1 : bob2.Index, INDEX$: '' + bob2.Index, ANIM$: bob2Param.Anim, FRAME: bob2Param.currentFrame, TOTALFRAMES: bob2Param.display.spriteSheet._data[ bob2Param.Anim ].frames.length }, bob2Param );
				}				
			}
			self.bobParams[ actName ] = bob2Param;
		}
				...
				if( self.keys[ bob2Param.controler.controls[ c ] ] )
				{
					...
					if( bob2Param.OnControl && bob2Param.OnControl != "" && self.keys[ bob2Param.controler.controls[ c ] ] != undefined && self.keys[ bob2Param.controler.controls[ c ] ] != false )
					{
						if( bob2Param.lastControl != bob2Param.controler.controls[ c ] )
						{
							...
							if( event != undefined )
							{
								self.aoz.runProcedure( bob2Param.OnControl, { EVENT$: "oncontrol", CONTROL$: bob2Param.controler.controls[ c ], INDEX: isNaN( bob2.Index ) ? -1 : bob2.Index, INDEX$: '' + bob2.Index, BUTTON: ( event.button ) ? event.button : -1, ALTKEY: ( event.altKey ) ? event.altKey : -1, CTRLKEY: ( event.ctrlKey ) ? event.ctrlKey : -1, SHIFTKEY: ( event.shiftKey ) ? event.shiftKey : -1, METAKEY: ( event.metaKey ) ? event.metaKey : -1, MOUSEX: self.aoz.fp2Int( self.aoz.getXMouse() ), MOUSEY: self.aoz.fp2Int( self.aoz.getYMouse() ) }, bob2Param );	
							}
	...
	if( bob2Param.hasLimits && bob2Param.Enable )
	{
		if( bob2Param.limits.x1 != undefined &&  bob2Param.limits.x1 != -1001 && bob2.vars.x < ( bob2Param.limits.x1 + hx ) )
		{
			...
			{
				self.aoz.runProcedure( bob2Param.OnLimit, { EVENT$: 'onlimit', LIMIT$: 'left', INDEX: isNaN( bob2.Index ) ? -1 : bob2.Index, INDEX$: '' + bob2.Index },bob2Param );
			}
		...
		if( bob2Param.limits.x2 != undefined &&  bob2Param.limits.x2 != -1001 && bob2.vars.x > bob2Param.limits.x2 -  hx  )
		{
			...
			if(  bob2Param.OnLimit && bob2Param.OnLimit != "" )
			{
				self.aoz.runProcedure( bob2Param.OnLimit, { EVENT$: 'onlimit', LIMIT$: 'right', INDEX: isNaN( bob2.Index ) ? -1 : bob2.Index, INDEX$: '' + bob2.Index },bob2Param );
			}
		...
		if( bob2Param.limits.y1 != undefined && bob2Param.limits.y1 != -1001 && bob2.vars.y < bob2Param.limits.y1 + hy )
		{
			...
			if(  bob2Param.OnLimit && bob2Param.OnLimit != "" )
			{
				self.aoz.runProcedure( bob2Param.OnLimit, { EVENT$: 'onlimit', LIMIT$: 'top', INDEX: isNaN( bob2.Index ) ? -1 : bob2.Index, INDEX$: '' + bob2.Index }, bob2Param );
			}
		...
		if( bob2Param.limits.y2 != undefined &&  bob2Param.limits.y2 != -1001 && bob2.vars.y > ( bob2Param.limits.y2 - hy ) )
		{
			...
			if( bob2Param.OnLimit && bob2Param.OnLimit != "" )
			{
				self.aoz.runProcedure( bob2Param.OnLimit, { EVENT$: 'onlimit', LIMIT$: 'bottom', INDEX: isNaN( bob2.Index ) ? -1 : bob2.Index, INDEX$: '' + bob2.Index }, bob2Param );
			}
		}
	}
	...
	{
		self.aoz.runProcedure( bob2Param.OnChange, { EVENT$: "onchange", INDEX: isNaN( bob2.Index ) ? -1 : bob2.Index, INDEX$: '' + bob2.Index, X: bob2.vars.x, Y: bob2.vars.y, ALPHA: bob2.vars.alpha, ANGLE: ( bob2.vars.angle * ( 180 / Math.PI ) ), SCALEX: bob2.vars.scaleX, SCALEY: bob2.vars.scaleY, HREV: bob2.vars.hRev, VREV: bob2.vars.vRev, SKEWX: bob2.vars.skewX, SKEWY: bob2.vars.skewY },bob2Param );
	}
	...
	if( actor_behaviors  )
	{
		var behavior = function()
		{
			...
			this.render = function( actor )
			{
				...
				{
					application.aoz.runProcedure( self.onRender, { BEHAVIOR$: options.name, STATE: self.state, INDEX$: actor.Id, KEY$: self.key } );
				}
/**
				var actName = ( !isNaN( actor.Id ) ) ? 'act_' + actor.Id : actor.Id;
