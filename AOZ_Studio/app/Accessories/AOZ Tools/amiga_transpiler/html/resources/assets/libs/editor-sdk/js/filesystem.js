	...
	getDirectory( path, procName )
	{
		...
		application.editorSDK.callRequest( 'filesystem', 'getDirectory', { path: path }, function( response )
        {
			...
			if( response )
			{
				...
				if( procName && procName != '' && response.files )
				{
					var size = 0;
					...
					application.root.vars._FS_NAME$_array.dimensions[ 0 ] = size;
					application.root.vars._FS_NAME$_array.array = [];
					application.root.vars._FS_PATH$_array.dimensions[ 0 ] = size;
					application.root.vars._FS_PATH$_array.array = [];
					application.root.vars._FS_TYPE_array.dimensions[ 0 ] = size;
					application.root.vars._FS_TYPE_array.array = [];
					for( var r = 0; r < response.files.length; r++ )
					{
						application.root.vars._FS_NAME$_array.array.push( response.files[ r ].name );
						application.root.vars._FS_PATH$_array.array.push( response.files[ r ].path );
						application.root.vars._FS_TYPE_array.array.push( response.files[ r ].type );
					}
