application.editorSDK.project =
{
	...
	getAOZFolder: function( procName )
    {
		application.root.vars.AOZ_FOLDER$ = "";
		application.editorSDK.busy( true );
		application.editorSDK.callRequest( 'project', 'getAOZFolder', {}, function( response )
        {
			if( response )
			{
	            if( response.error )
				...
				application.root.vars.AOZ_FOLDER$ = response.path;
				if( procName && procName != '' )
				{
					application.aoz.runProcedure( procName, { EVENT$: 'onget', PATH$: response.path } );
				}
