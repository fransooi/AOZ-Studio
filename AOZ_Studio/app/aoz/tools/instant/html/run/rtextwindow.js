...
function TextWindow( aoz, screen, contextName, definition, defaultWindow, tags )
{
	...
	if ( !defaultWindow )
	{
		...
		else
		{
			...
			var self = this;
			this.aoz.loadingMax++;
			this.aoz.fonts.getFont( definition.forceFont.name, definition.forceFont.height, definition.forceFont.weight, definition.forceFont.italic, definition.forceFont.stretch, '', definition.forceFont.context, function( response, data, extra )
			{
				self.aoz.loadingCount++;
				self.font = data;
			} );
			this.fontHeight = screen.vars.height / this.height;
			//this.fontWidth =  this.aoz.manifest.default.screen.width / this.aoz.manifest.default.screen.window.width;
			//this.fontHeight *= ( screen.vars.height / this.aoz.manifest.default.screen.height );
			this.fontWidth = this.fontHeight / 1.8;	//( screen.vars.width / this.aoz.manifest.default.screen.width );
		}
		this.fontString = this.utilities.getFontString( this.font.fontInformation.name, this.fontHeight, definition.font.weight, definition.font.italic, definition.font.stretch );
	}
	else
	{
...
TextWindow.prototype.print = function( text, newLine, centre )
{
	...
	if ( this.aoz.directMode || this.aoz.section.isDebuggerCommand )
	{
		...
		this.aoz.toPrintToDirectMode += ( text + ( newLine ? '\n' : '' ) );
		return;
	}
	...
	if ( this.font.fontInformation.type == 'google' )
	{
		if ( ( writing & ( TextWindow.FLAG_NORMAL | TextWindow.FLAG_ONLYPEN ) ) != 0 )
		{
			...
			if ( ( writing & TextWindow.FLAG_OUTLINE ) != 0 )
			{
				...
				{
					context.strokeStyle = colorPen;
					strokeThisText( line, x + disp, yu, width );
				}
			}
			else
				fillThisText( line, x + disp, y, width );
