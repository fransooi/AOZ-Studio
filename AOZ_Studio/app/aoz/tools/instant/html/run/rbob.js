	...
	if ( this.limits )
	{
		...
		if ( typeof position.y != 'undefined' && typeof this.limits.height != 'undefined' )
		{
			if ( this.imageObject )
				...
				position.y = Math.max( this.limits.y, Math.min( position.y, this.limits.y2 ) );
		}
	}
...
Bob.prototype.updateCollisionData = function( active )
{
	if ( this.vars.modified != this.modifiedCollisions )
	{
		...
		if ( this.imageObject )
		{
			...
			{
				collisions.xCoord = ( this.screen.vars.x - this.screen.vars.hotspotX - this.aoz.renderer.getHardLeftX() ) + ( this.vars.x - this.screen.vars.offsetX ) * collisions.renderScaleX;
				collisions.yCoord = ( this.screen.vars.y - this.screen.vars.hotspotY - this.aoz.renderer.getHardTopY() ) + ( this.vars.y - this.screen.vars.offsetY ) * collisions.renderScaleY;
				collisions.x = collisions.xCoord - collisions.hotSpotX;
				collisions.y = collisions.yCoord - collisions.hotSpotY;
			}
			else
			{
				var xScreen = this.screen.vars.x - this.screen.vars.hotspotX - this.aoz.renderer.getHardLeftX();
				var yScreen = this.screen.vars.y - this.screen.vars.hotspotY - this.aoz.renderer.getHardTopY();
