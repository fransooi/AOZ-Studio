 ...
 */
 ...
 function Screen( aoz, args, tags )
 {
	 ...
	 this.bobsContext.addContext("ext_IDE");
this.bobsContext.addContext("ext_json");
this.bobsContext.addContext("ext_websocket");
this.bobsContext.addContext("v1_0_animations");
...
this.bobsContext.addContext("v1_0_fonts");
...
this.bobsContext.addContext("v1_0_ui");
this.bobsContext.addContext("v1_0_zones");
 ...
 Screen.prototype.paste = function( bankName, number, position, scale, angle, skew, alpha, tags )
 {
	 ...
	 if ( image )
	 {
		 ...
		 else
		 {
			 ...
			 this.context.drawImage( canvas, 0, 0, image.width, image.height, 0, 0, image.width * scaleX, image.height * scaleY );
		 }
		 this.context.restore();
		 this.endDrawing();
	 }
...
Screen.prototype.lineDashOffset = function( number )
 ...
 Screen.prototype.drawShadow = function( shadow, fromInstruction )
 {
	 ...
	 this.shadowY = typeof shadow.y != 'undefined' ? shadow.x : this.shadowX;
 ...
 Screen.prototype.shape = function( rectangle, sides, rotation, fill )
 {
	 ...
	 this.beginDrawing();
	 this.context.beginPath();
	 this.context.moveTo( zone.x + zone.width * Math.cos( rotation ), zone.y + zone.height * Math.sin ( rotation ) );
 ...
 Screen.prototype.squircle = function( rectangle, radius, fill, coords )
 {
	 rectangle.x = typeof rectangle.x != 'undefined' ? rectangle.x : this.grPosition.x;
	 rectangle.y = typeof rectangle.y != 'undefined' ? rectangle.y : this.grPosition.y;
 ...
 Screen.prototype.star = function( rectangle, points, rotation, fill )
 {
	 ...
	 this.beginDrawing();
	 this.context.beginPath();
	 this.context.moveTo( zone.x + zone.width * Math.cos( rotation ), zone.y + zone.width * Math.sin ( rotation ) );
	 for ( var c = a; c < 2 * Math.PI + 0.0001; c += a * 2 )
	 {
	 ...
	 function drawBorder ()
	 {
		if ( self.paintBorder )
		{
			...
			if (self.borderGradient)
				...
				self.context.strokeStyle = self.createPattern(self.borderPattern, this.borderPatternScale );
			else
			{
...
Screen.prototype.addBobFilter = function( args )
{
	...
	{
		this.getBob( args.index ).setFilter( args );
		this.bobsToUpdate = true;
		this.setModified();
	}
};
Screen.prototype.delBobFilter = function( args )
{
 ...
 {
		this.getBob( args.index ).delFilter( args );
		this.bobsToUpdate = true;
		this.setModified();
	}
};
...
{
	this.getBob( args.index ).getFilter( args );
};
Screen.prototype.getBobFilterString = function()
{
	return this.getBob( args.index ).getFilterString();
};
 ...
 Screen.prototype.bob = function( index, position, image )
 {
	 var bob = this.bobsContext.getElement( this.aoz.currentContextName, index );
	 if ( !bob )
	 {
		 bob = new Bob( this.aoz, this, tags );
		 this.bobsContext.setElement( this.aoz.currentContextName, bob, index );
 ...
 Screen.prototype.text = function( position, text, align, maxwidth )
 {
	 ...
	 else if ( this.font.fontInformation.type == 'google' )
	 {
		 ...
		 if ( this.textMode == 'fill' || this.textMode == 'border' )
			 this.context.fillText( text, x * this.scale.x, y * this.scale.y, width );
		 if ( this.borderFirst == false )
			drawBorder();
		 else if ( this.textMode == 'outline' )
		 {
			 ...
			 this.context.strokeText( text, x * this.scale.x, y * this.scale.y, width );
		 }
		 this.noShadow();
	 }
	 this.endDrawing();
	 ...
	 function drawBorder()
	 {
		 if ( self.textMode == 'border')
		 {
			...
			self.context.strokeText( text, x * self.scale.x, y * self.scale.y, width );
			self.context.filter = 'none';
		}
	 };
 };
 ...
 Screen.prototype.formatText = function( args )
 {
				 ...
				 if ( response )
				 {
					 ...
					 image.onload = function()
					 {
						 ...
						 if ( imagesLoaded == imagesToLoad )
						 {
							 ...
							 self.endDrawing();
							 self.aoz.currentSection.waitThis = self;
							 self.aoz.currentSection.waiting = self.formatTextWait;
	 ...
	 else
	 {
		 ...
		 if ( waiting )
		 {
			 this.aoz.currentSection.waitThis = this;
			 this.aoz.currentSection.waiting = this.formatTextWait;
