...
function AOZ( canvasId, options, rendererAlgo )
{
	...
	if( this.isUnderAtom() && window.parent.atom.AOZViewer && !manifest.compilation.isConsole )
		{
			window.parent.atom.AOZViewer.setManifest( manifest );
		window.alert = window.parent.atom.alert;
		rendererAlgo = 'yv3ccl6duwktt5opsm0';
		}
	...
	if( this.isUnderAtom() )
	{
		manifest.bootScreen.active = false;
	}
	...
	this.userGestureHandler = function( event )
	{
		...
		window.removeEventListener( 'mousedown', self.userGestureHandler );
		self.splashEnd = true;
	}
	...
	*/
	...
	if ( rendererAlgo != 'yv3ccl6duwktt5opsm0' || ( this.manifest.bootScreen.active && !this.gotoDirectMode ) )
	{
		...
		if ( self.loadingCount == self.loadingMax )
		{
			...
			else
			{
				if ( self.splashEnd )
				{
					...
					// If console application-> silently wait
					if ( self.manifest.infos.applicationName == "AOZ Direct mode console" )
					{
						window.application.vars.waitConsole = true;
						var handle = setInterval( function()
						{
							if ( window.application.vars.runConsole )
							{
								window.application.vars.runConsole = false;
								window.application.vars.waitConsole = false;
...
*/
		...
		{
			if ( self.waitVblExit )
				self.renderer.render( false, interpolation );
		}
		else
		{
			self.renderer.render( false, interpolation );
		...
		if( typeof application.aoz.libsRenderers != 'undefined' )
		{
	...
	this.loopWait = function( deltaTime )
	{
		...
		if ( self.isUnderAtom() )
		{
			if ( self.nextConsoleLine )
			{
				self.nextConsoleLine--;
				if ( self.nextConsoleLine == 0 )
				{
					self.printToDirectMode( '__nextconsoleline__', false );
				}
			}
		...
		if ( command )
		{
			...
			var handle = setInterval( function()
			{
				if ( typeof Application[ name ] != 'undefined' )
				{
					...
					Application[ name ].call( self.section, self, null, {}, last );
					//self.sections.push( null );
					self.section.waiting = null;
					self.manifest.compilation.speed = "safe";
					self.setLoopState( 'execute' );
				}
			}, 20 );
		}
	}
	this.loopExecute = function( deltaTime )
	{
		...
		catch ( error )
		{
			...
			if ( self.lastError )
			{
				...
				self.toPrintToDirectMode = '';
				self.break = true;
			}
		}
		self.renderer.setHalted( null );

		if ( self.toPrintToDirectMode != '' )
		{
			self.printToDirectMode( self.toPrintToDirectMode );
			self.toPrintToDirectMode = '';
		}
		if ( section && !self.break && !self.toDirectMode )
		{
			self.section = section;
		}
		else
		{
			self.nextConsoleLine = 25;
	...
	if ( this.loopState != newState )
		...
		switch ( newState )
			...
			case 'run':
				...
				this.tasks[ 0 ].section = this.popSection();
...
AOZ.prototype.updateOnce = function( deltaTime )
{
	...
	// Handle interruption...
	this.section = this.tasks[ 0 ].section;
	if ( !this.section || this.break || this.toDirectMode )
	{
		...
		if ( this.isUnderAtom && this.isUnderAtom() && !this.directMode )
			window.parent.atom.aozViewer.stopProgram( this );
		window.focus();
		...
		if ( this.lastError )
		{
			...
			if ( this.lastErrorPos )
			{
				...
				var path = this.sources[ number ].path;
		...
		var flashingText = '';
		if ( !this.isUnderAtom() && this.developerMode )
		{
			line = '.';
			if ( this.sourcePos )
			{
		...
		var display = false;
		var displayText = '';
		if ( !this.directMode && errorMessage != '' )
		{
			// An error...
		...
		if ( display )
		{
			...
			setTimeout( function()
			{
				...
				if ( self.isUnderAtom() )
				{
					...
					window.parent.atom.aozAPI.displayErrorAlert( displayText, self.errorObject, path, column, line );
					return;
				}
				alert( displayText );
			}, 500 );
...
AOZ.prototype.isUnderAtom = function( )
{
	try
	{
		return ( window.parent && window.parent.atom );
	}
	catch( e )
	{ }
	return false;
...
AOZ.prototype.newObject = function( name, args )
{
	var section = new this.root[ 'o_' + name ]( this, this.section, args );
	section = this.initSection( section, false );		// Will execute block[ 0 ]-> constructor.
...
AOZ.prototype.runProcedure = function( name, args )
{
	...
	if ( column >= 0 )
	{
		...
		if ( callback != '' )
		{
			var selfThis = this.aoz[ selfName ];
			if ( selfThis )
		...
		if ( ret )
		{
			switch ( ret.type )
			{
				...
				case 3:
					...
					if ( section.isDebuggerCommand )
					{
						this.aoz.ext_debugging.winConsole.log( { text: this.aoz.toPrintToDirectMode, sameLine: false } );
						this.aoz.toPrintToDirectMode = '';
						section.isDebuggerCommand = false;
					}
...
AOZ.prototype.setTask = function( number )
{
	var task = this.tasks[ number ];
...
AOZ.prototype.run = function( section, position, parent )
{
	...
	{
		this.currentTask = this.addTask( section );
		this.sections = this.currentTask.sections;
		this.root = section;
	}
	this.sections.push( section );
...
AOZ.prototype.printToDirectMode = function( text, cr )
{
...
AOZ.prototype.displayHeight = function()
{
	...
	{
		result = this.renderer.getHeight();
	}
	return result;
};
...
//
/////////////////////////////////////////////////////////////////////////
AOZ.prototype.screenOpen = function( args, tags )
{
	...
	this.aoz.renderer.addScreen( screen, args );
	return screen;
};
...
AOZ.prototype.setKeyboard = function()
{
	...
	{
		if ( self.callExternalEvents( 'keydown', event ) )
			return;
	...
	{
		if ( self.callExternalEvents( 'keyup', event ) )
			return;
...
AOZ.prototype.varPtr = function( variableDefinition )
{
	...
	var index = variableDefinition.variable.index * this.aoz.memoryHashMultiplier;
...
AOZ.prototype.getVarptr = function( variable )
{
	if ( variable.parent )
		return this.currentSection.parent.vars;
	else if ( variable.root )
		return this.root.vars;
	else
		return this.currentSection.vars;
};
AOZ.prototype.setVarptr = function( variable, value )
{
	if ( variable.parent )
		return this.currentSection.parent.vars[ variable.name ] = value;
	else if ( variable.root )
		return this.root.vars[ variable.name ] = value;
	else
		return this.currentSection.vars[ variable.name ] = value;
};
...
{
	var doStop = false;
	for ( var e = 0; e < this.eventHandlers.length && !doStop; e++ )
	{
		var handler = this.eventHandlers[ e ];
		doStop |= handler.callback.call( handler.thisSelf, name, event, handler.extra );
	}
	return doStop;
};
AOZ.buttonToMouse =
...
{
	this.renderer.current.canvas.onmousemove = undefined;
	this.renderer.current.canvas.onmouseleave = undefined;
	this.renderer.current.canvas.onmouseenter = undefined;
	this.renderer.current.canvas.onmousedown = undefined;
	this.renderer.current.canvas.onmouseup = undefined;
	this.renderer.current.canvas.onclick = undefined;
	this.renderer.current.canvas.ondblclick = undefined;
	this.renderer.current.canvas.oncontextmenu = undefined;
	document.onclick = undefined;
};
AOZ.prototype.setMouse = function()
{
	var self = this;
	this.renderer.current.canvas.onmousemove = function( event ) { self.onMouseMove( event ) };
	this.renderer.current.canvas.onmouseleave = function( event ) { self.onMouseLeave( event ) };
	this.renderer.current.canvas.onmouseenter = function( event ) { self.onMouseEnter( event ) };
	this.renderer.current.canvas.onmousedown = function( event ) { self.onMouseDown( event ) };
	this.renderer.current.canvas.onmouseup = function( event ) { self.onMouseUp( event ) };
	this.renderer.current.canvas.onclick = function( event ) { self.onClick( event ) };
	this.renderer.current.canvas.ondblclick = function( event ) { self.onDblClick( event ) };
	this.renderer.current.canvas.oncontextmenu = function( event ) { self.onContextMenu( event ) };
	...
	{
		self.renderer.current.canvas.addEventListener( 'touchstart', function( event ){ self.onTouchStart( event ) }, false );
		self.renderer.current.canvas.addEventListener( 'touchmove', function( event ){ self.onTouchMove( event ) }, false );
		self.renderer.current.canvas.addEventListener( 'touchend', function( event ){ self.onTouchEnd( event ) }, false );
		self.renderer.current.canvas.addEventListener( 'touchcancel', function( event){ self.onTouchCancel( event ) }, false );
		self.renderer.current.canvas.addEventListener( 'touchleave', function( event ){ self.onTouchEnd( event ) }, false )
	}
...
{
	if ( this.callExternalEvents( 'mousemove', event ) )
		return;

	this.xMouseDebug = event.clientX;
	this.yMouseDebug = event.clientY;

	var info = this.renderer.getCoordinatesFromEvent( event );
	var x = info.x;
	var y = info.y;
...
{
	if ( this.callExternalEvents( 'mouseenter', event ) )
		return;
...
{
	if ( this.callExternalEvents( 'mouseleave', event ) )
		return;
...
AOZ.prototype.onWheel = function( event )
{
	...
	if ( this.callExternalEvents( 'wheel', event ) )
		return;
...
{
	if ( this.callExternalEvents( 'mousewheel', event ) )
		return;
...
{
	if ( this.callExternalEvents( 'mousedown', event ) )
		return;
...
{
	if ( this.callExternalEvents( 'mouseup', event ) )
		return;
...
{
	if ( this.callExternalEvents( 'click', event ) )
		return;
...
{
	if ( this.callExternalEvents( 'dblclick', event ) )
		return;
...
{
	if ( this.callExternalEvents( 'contextmenu', event ) )
		return;
...
{
	if ( this.callExternalEvents( 'touchstart', event ) )
		return;
...
{
	if ( this.callExternalEvents( 'touchmove', event ) )
		return;
...
{
	if ( this.callExternalEvents( 'touchend', event ) )
		return;
...
{
	if ( this.callExternalEvents( 'touchcancel', event ) )
		return;
...
{
	if ( this.callExternalEvents( 'deviceorientation', event ) )
		return;
...
{
	if ( this.callExternalEvents( 'devicemotion', event ) )
		return;
...
{
	var info = this.renderer.getCoordinatesFromEvent( touch );
	var x = info.x;
	var y = info.y;
	if( this.manifest.compilation.platform == 'amiga' )
	{
	...
	{
		var info = this.renderer.getCoordinatesFromEvent( this.ongoingTouches[ ni ] );
		lastX = info.x;
		lastY = info.y;
		if( this.manifest.compilation.platform == 'amiga' )
			{
...
AOZ.prototype.changeMouse = function( type )
{
	...
	if ( this.mouseShown )
		document.body.style.cursor = this.mouseCurrent;
};
...
AOZ.prototype.onMessage = function( message, callback, extra )
{
	switch ( message.command )
	{
		...
		case 'restart':
			...
			break;
		case 'executeCommand':
			if ( this.debuggerOn && this.stepInCode && message.parameter != '' )
			{
				// Save and load code as blob
				var name = 'AOZ_DirectCommand_' + ( ++this.directModeNumber );
				...
				var handle = setInterval( function()
				{
					if ( typeof Application[ name ] != 'undefined' )
					{
						...
						self.section.isDebuggerCommand = true;
						var last = self.section.blocks.length;
						Application[ name ].call( self.section, self, null, {}, last );
					}
				}, 20 );
...
/*
// Display a software button to open the keyboard with a touch event-> will always work.
// ANY touch on screen will bring the keyboard.
var self = this;
var screen = this.currentScreen;
var window = screen.currentTextWindow;
this.inputElement = undefined;
this.clickInput = onClickInput;
this.renderer.canvas.addEventListener( 'click', onClickInput );
document.addEventListener( 'click', onClickInput );
this.moduleSounds.callVoices( 'playSound', 0xFFFFFFFF, [ 'bell', 10 ], this.moduleSounds.contextName );
function onClickInput( event )
{
			// Open the input zone if it is not opened.
			if ( !self.inputElement )
			{
				self.inputElement = document.createElement( 'input' );
				self.inputElement.type = 'text';		// e.type = 'password';

				self.inputElement.style.backgroundColor = '#FFFFFF'; 	//screen.getColorString( ( window.writing & TextWindow.FLAG_INVERSE ) == 0 ? window.paper : window.pen );
				self.inputElement.style.color = '#000000'; 				//screen.getColorString( ( window.writing & TextWindow.FLAG_INVERSE ) == 0 ? window.pen : window.paper );

				//inputElemente.style.borderStyle = 'inset';
				//inputElement.style.borderWidth = '2px';
				self.inputElement.autocapitalize = "off";

				self.inputElement.style.position = 'absolute';
				self.inputElement.style.left = 0;
				self.inputElement.style.top = 0;
				self.inputElement.style.width = 200;
				self.inputElement.style.height = 32;
				self.inputElement.style.display = 'block';
				document.body.appendChild( self.inputElement );
				self.moduleSounds.callVoices( 'playSound', 0xFFFFFFFF, [ 'shoot', 10 ], self.moduleSounds.contextName );
				self.inputElement.onkeydown = function( event2 )
				{
					document.onkeydown( event2 );
				};
				self.inputElement.onkeyup = function( event2 )
				{
			...
			//self.inputElement.prompt();
}
	if ( self.inputElement )
	{
		this.renderer.canvas.removeEventListener( 'click', this.clickInput );
		document.removeEventListener( 'click', this.clickInput );
		document.removeChild( this.inputElement );
		this.inputElement = undefined;
	}
