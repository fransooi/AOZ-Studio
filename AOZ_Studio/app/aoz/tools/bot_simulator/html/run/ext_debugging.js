...
this.blocks=[];
	...
	{
		// From source: C:/AOZ_Studio/AOZ_Studio/app/aoz/extensions/debugging/debugging.aoz
		...
		this.consoleOn = function( args )
		{
			var newArgs = 
			{
				...
				consolePrompt: args.consolePrompt,
				mode: args.wait ? 'stepwait' : 'play',
				noHelpers: args.noHelpers
			};
		...
		this.consoleVisible = function( args )
		{
			if ( this.initializedConsole )
			{
				this.winConsole.visible = args.visible;
			}
		}
		this.consoleInterpret = function( args )
		{
			if ( this.initializedConsole )
			{
				if ( args.callback )
				{
					var self = this;
					this.winConsole.interpretCommand( args.command, function( response, data, extra )
					{
						if ( response )
						{
				...
				this.winConsole.interpretCommand( args.command );
			}
		}
		...
		this.initialize = function( args, callback, extra )
		{
			if ( !this.initialized && !this.initializing )
			{
				...
				function doInit()
				{
					...
					self.vars.masks = ( typeof args.masks == 'undefined' ? self.vars.masks : ( args.noHelper ? false : args.masks ) );
					self.vars.crosshair = ( typeof args.crosshair == 'undefined' ? self.vars.crosshair : ( args.noHelper ? false : args.crosshair ) );
					self.vars.screens = ( typeof args.screens == 'undefined' ? self.vars.screens : args.screens );
					self.vars.collisions = ( typeof args.collisions == 'undefined' ? self.vars.collisions : ( args.noHelper ? false : args.collisions ) );
					self.vars.rulers = ( typeof args.rulers == 'undefined' ? self.vars.rulers : ( args.noHelper ? false : args.rulers ) );
					self.vars.grid = ( typeof args.grid == 'undefined' ? self.vars.grid : ( args.noHelper ? false : args.grid ) );
					...
					self.vars.noHelpers = ( typeof args.noHelpers == 'undefined' ? self.vars.noHelpers : args.noHelpers );
					...
					self.vars.consolePrompt = ( typeof args.consolePrompt == 'undefined' ? self.vars.consolePrompt : args.consolePrompt );
		...
		{
			return ( this.initialized && !this.initializedConsole && this.vars.visible );
		}
		...
		this.ConsoleWindow.prototype.interpretCommand = function( command, callback, extra )
		{
			if ( command != '' )
			{
				var self = this;
				...
				if ( this.debugger.debugEvents.isConnected() )
				{
					...
					}, function( response, extra )
					{
						if ( !response.error )
						{
							self.aoz.onMessage( { command: 'executeCommand', parameter: response.code, callback: callback, extra: extra } );
							if ( !callback && self.debugger.vars.consoleOutput != 'debugger' )
								self.aoz.ext_source_editor.nextConsoleLine( { index: 'console' } );
						}
						else
						{
							if ( callback )
								callback( false, response.code, extra );
		...
		this.ApplicationWindow.prototype.onMouse = function( eventName, event, winAbove )
		{
			while ( this.mouseIn )
			{
				...
				if ( !this.debugger.vars.noHelpers )
				{
				if ( !this.editedMask )
				{
						...
						if ( !this.editedMask )
					{
						...
						function( screen )
						{
							...
							screen.bobsContext.parseAll( undefined, function( bob )
							{
								if ( !bob.collisions.active )
								{
									...
									if ( bob.collisions.active && !self.debugger.vars.noHelpers )
								{
						...
						this.aoz.sprites.context.parseAll( undefined, function( sprite )
						{
							if ( !sprite.collisions.active )
							{
								...
								if ( !sprite.collisions.active && !self.debugger.vars.noHelpers )
							{
								var ink = self.debugger.getInk( 'bobMask', self.debugger.INK_NORMAL );
		...
		this.ApplicationWindow.prototype.drawBobLabel = function( bob, coordinates )
		{
			...
			width = Math.min( this.screen.textLength( text ) + fontHeight * 2, 250 );
			...
			var rect = { x: coordinates.x - width - 8, y: coordinates.y - height - 8, width: width, height: height };
