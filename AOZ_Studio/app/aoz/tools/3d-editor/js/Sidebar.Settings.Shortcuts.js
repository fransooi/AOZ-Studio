...
function SidebarSettingsShortcuts( editor ) {
	...
	const shortcuts = [ 'translate', 'rotate', 'scale', 'undo', 'focus', 'clone' ];
	...
	document.addEventListener( 'keydown', function ( event ) {
		...
		switch ( event.key.toLowerCase() ) {
			...
			case config.getKey( 'settings/shortcuts/clone' ):
				console.log( 'ici' );
				const object2 = editor.selected;
				if ( object2 === null || object2.parent === null ) return; // avoid cloning the camera or scene
				var object3 = object2.clone();
				editor.execute( new AddObjectCommand( editor, object3 ) );
